...
MeshTest::InstallInternetStack ()
 ...
 MeshTest::InstallApplication ()
 {
   ...
   pingapp->SetAttribute ("Remote", Ipv4AddressValue ("192.168.13.11"));
   pingapp->SetAttribute ("Verbose", BooleanValue (true) );
   nodes.Get (sinkNodeId)->AddApplication (pingapp);
   pingapp->SetStartTime (Seconds (1.0));
   pingapp->SetStopTime (Seconds (m_totalTime + 1.5));

   std::ostringstream pingName;
   pingName << "pingapp-" << interfaces.GetAddress (sinkNodeId);
   Names::Add (pingName.str (), pingapp);
   Config::Connect ("/Names/" + pingName.str () + "/Rtt", MakeCallback (&PingRtt));
   // Names::Add ("pingapp-10.1.1.1", pingapp);
   // Config::Connect ("/Names/pingapp-10.1.1.1/Rtt", MakeCallback (&PingRtt));
 }
 int
 MeshTest::Run ()
 {
