...
{
    NS_LOG_UNCOND("Set TX attenuation to " << atten << " at time "
                                           << Simulator::Now().As(Time::Unit::S));
...
main(int argc, char* argv[])
{
    ...
    Ipv4Address target_addr("192.168.13.4");
    ...
    uint16_t port = 2277;
    BulkSendHelper source("ns3::TcpSocketFactory", InetSocketAddress(target_addr, port));
    ...
    sourceApps.Stop(Seconds(time+5));
