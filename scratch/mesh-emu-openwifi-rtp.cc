 ...
 * 0 - 1 - 2
 *
 * Mesh + Emu gateway with H.264 RTP video streaming to an external receiver.
 *
 * Receiver (192.168.13.12): python3 rtp-server.py
 * Video file: Annex-B .h264 (extract with ffmpeg -i video.mp4 -an -c:v libx264 -f h264 out.h264)
 */
...
/**
 * Replies to 192.168.13.11 often arrive with the host NIC MAC (OTHERHOST).
...
/**
 * UDP checksum covers a pseudo-header that includes the source IP.
 * After SNAT the checksum must match the post-NAT IP header.  We patch the
 * serialized bytes directly because UdpHeader::Serialize keeps a non-zero
 * m_checksum from Deserialize and skips recomputation.
 */
static Ptr<Packet>
...
class MeshGatewayNatRouting : public Ipv4RoutingProtocol
 ...
 private:
                            ...
                            const Ipv4Header &header)
   {
     ...
     if (IsOutsideEgress (route, m_outside))
       {
         ...
         if (hdr.GetProtocol () == UdpL4Protocol::PROT_NUMBER)
           {
             ...
             if (++patchLog <= 3)
               {
                 std::cout << "Gateway UDP checksum patched for "
 ...
 NS_LOG_COMPONENT_DEFINE ("MeshRtpExample");
 ...
 private:
  ...
  std::string m_rtpRemote; ///< RTP/UDP destination (GStreamer host on LAN)
  uint16_t  m_rtpPort; ///< RTP/UDP destination port
  uint32_t  m_rtpNode; ///< Mesh node that sends RTP video
  std::string m_videoFile; ///< Annex-B H.264 elementary stream
  double    m_fps; ///< Video frame rate for RTP timestamps
  double    m_configInterval; ///< In-band SPS/PPS interval (rtph264pay config-interval)
 ...
 MeshRtpTest::MeshRtpTest () :
  ...
  m_pcapIp (true),
  m_ascii (false),
  m_pcapPrefix ("mesh-rtp-gateway"),
  m_stack ("ns3::Dot11sStack"),
  m_root ("ff:ff:ff:ff:ff:ff"),
  m_rtpRemote ("192.168.13.11"),
  m_rtpPort (5004),
  m_rtpNode (8),
  m_videoFile ("naruto.h264"),
  m_fps (25.0),
  m_configInterval (1.0),
  m_enablePing (false),
  m_pingRemote ("192.168.13.11"),
  m_pingNode (8),
  m_emuDev (0)
{
}
 void
 MeshRtpTest::Configure (int argc, char *argv[])
 {
  ...
  cmd.AddValue ("ascii",   "Enable Ascii traces on interfaces", m_ascii);
   ...
   cmd.AddValue ("rtp-remote", "RTP/H264 destination IP (GStreamer receiver)", m_rtpRemote);
   cmd.AddValue ("rtp-port", "RTP/UDP destination port", m_rtpPort);
   cmd.AddValue ("rtp-node", "Mesh node id that streams H.264 RTP", m_rtpNode);
   cmd.AddValue ("video-file", "Annex-B H.264 elementary stream file path", m_videoFile);
   cmd.AddValue ("fps", "Video frame rate for RTP timestamps", m_fps);
   cmd.AddValue ("config-interval", "Seconds between in-band SPS/PPS bursts", m_configInterval);
...
void
MeshRtpTest::InstallInternetStack ()
 {
 ...
 void
 MeshRtpTest::InstallApplication ()
 {
   NS_ABORT_MSG_IF (m_rtpNode >= nodes.GetN (), "rtp-node out of range");

   NS_LOG_INFO ("Install RtpH264Client on node " << m_rtpNode << " -> "
                << m_rtpRemote << ":" << m_rtpPort);
   Address rtpRemote = InetSocketAddress (Ipv4Address (m_rtpRemote.c_str ()), m_rtpPort);
   RtpH264ClientHelper rtpClient (rtpRemote);
   ApplicationContainer rtpApps = rtpClient.Install (nodes.Get (m_rtpNode));
   Ptr<RtpH264Client> rtp = rtpApps.Get (0)->GetObject<RtpH264Client> ();
   rtp->SetAttribute ("VideoFile", StringValue (m_videoFile));
   rtp->SetAttribute ("Fps", DoubleValue (m_fps));
   rtp->SetAttribute ("ConfigInterval", DoubleValue (m_configInterval));
   rtp->SetAttribute ("PayloadType", UintegerValue (96));
   rtp->SetAttribute ("MaxPackets", UintegerValue (0));
   rtpApps.Start (Seconds (1.0));
   rtpApps.Stop (Seconds (m_totalTime + 1.0));
   std::cout << "H.264 RTP: node " << m_rtpNode << " -> " << m_rtpRemote << ":" << m_rtpPort
             << " file=\"" << m_videoFile << "\" @ " << m_fps << " fps"
             << " for ~" << m_totalTime << "s"
             << " (receiver: python3 rtp-server.py on " << m_rtpRemote << ")" << std::endl;
 ...
 MeshRtpTest::Run ()
 {
  ...
  EnablePcapTraces ();
  InstallApplication ();
   Simulator::Schedule (Seconds (m_totalTime), &MeshRtpTest::Report, this);
