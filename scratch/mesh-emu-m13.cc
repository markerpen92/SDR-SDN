 ...
 * mesh-emu-m13.cc
 *
 * Dual-gateway mesh transit: external RTP enters via br0 on node 0,
 * crosses the mesh to node 9, and exits via sdr0 to 192.168.13.0/24.
 *
 *   192.168.10.x (LAN) --br0--> [Node 0  NAT-1 DNAT]
 *                                    |
 *                               mesh 10.1.1.0/24
 *                                    |
 *   192.168.13.x (LAN) <--sdr0-- [Node 9  NAT-2 SNAT]
 *
 ...
 *   Terminal A: ./ns3 run mesh-emu-m13 -- --enable-ping=0 --pcap=0
 *   Terminal B (.188): python3 rtp-server.py
 *   Terminal C (.4.x): gst-launch ... udpsink host=192.168.10.111 port=5004
 *     same PC without ip neigh: udpsink host=192.168.10.255 port=5004
 *   Node 0 ingress DNAT 4.111:5004 -> 192.168.13.188, mesh -> node 9 SNAT -> sdr0.
 */
...
/**
 * Node 0 ingress RTP/H.264 observer: FPS + I-frame (IDR) counting.
 */
class IngressRtpStats
{
public:
  struct Sample
  {
    ...
    if (info.idr)
      {
        ...
        if (++idrLog <= 30 || (idrLog % 50) == 0)
          {
            std::cout << "[node0 RTP I-frame] seq=" << info.seq
...
private:
  void EnsureTimer ()
  {
    ...
    m_reportEvent = Simulator::Schedule (Seconds (m_nextSampleTime), &IngressRtpStats::Report, this);
  }
    ...
    if (!m_samples.empty ())
      {
        ...
        std::cout << "[node0 RTP stats] t=" << last.timeS << "s FPS=" << last.fps
  ...
  void WriteCsv () const
  {
    if (m_samples.empty ())
      {
        std::cerr << "[node0 RTP stats] no samples; CSV not written\n";
        return;
      }
    std::ofstream out (m_csvPath.c_str ());
    if (!out.is_open ())
      {
        std::cerr << "[node0 RTP stats] failed to open " << m_csvPath << " for write\n";
        return;
      }
    ...
    out.close ();
    std::cout << "[node0 RTP stats] wrote " << m_samples.size () << " rows to "
              << m_csvPath << std::endl;
  }
...
static IngressRtpStats g_ingressRtpStats;
    ...
    if (m_outside && idev == m_outside && hdr.GetFragmentOffset () == 0)
      {
        const bool rtpIngressTrigger =
          (hdr.GetProtocol () == UdpL4Protocol::PROT_NUMBER && m_doIngressRtpDnat
           && IsIngressRtpTrigger (hdr.GetDestination (), m_outsideAddr));

        if (m_doIngressDnat && hdr.GetDestination () == m_outsideAddr)
          {
                ...
                if (ReadIcmpEchoSeq (packet, Icmpv4Header::ICMPV4_ECHO, seq))
                  {
                    std::cout << "[" << m_gatewayLabel << " ICMP ingress DNAT] "
                              ...
                              << " seq=" << seq << " => " << m_ingressDnatDest << std::endl;
        ...
        if (rtpIngressTrigger)
          {
            ...
            if (PeekUdpDestPort (packet, dport) && dport == m_rtpPort)
              {
                g_ingressRtpStats.Observe (packet);
        ...
        else if (m_hasTransitNet && IsTransitDest (hdr.GetDestination ()))
          {
            if (hdr.GetProtocol () == 1)
              {
                uint16_t seq = 0;
...
private:
  ...
  bool      m_rtpIngressStats;
  std::string m_rtpStatsCsv;
...
MeshM13Test::MeshM13Test ()
  : m_xSize (4),
    ...
    m_rtpIngressStats (true),
    m_rtpStatsCsv ("node0-rtp-stats.csv"),
...
MeshM13Test::Configure (int argc, char *argv[])
{
  ...
  cmd.AddValue ("rtp-ingress-stats", "FPS + I-frame stats on node 0 ingress", m_rtpIngressStats);
  cmd.AddValue ("rtp-stats-csv", "CSV output path for node 0 FPS/throughput", m_rtpStatsCsv);
  cmd.AddValue ("rtp-stats-interval", "Seconds between CSV samples", m_rtpStatsInterval);
...
MeshM13Test::InstallInternetStack ()
{
  ...
  g_ingressRtpStats.Enable (m_rtpIngressStats && !m_rtpLocalParse);
  if (m_rtpIngressStats && !m_rtpLocalParse)
    {
      g_ingressRtpStats.SetCsvPath (m_rtpStatsCsv);
      g_ingressRtpStats.SetSampleInterval (m_rtpStatsInterval);
    }
  ...
  std::cout << "\n=== External terminal ping (transit through mesh) ===\n"
            ...
            << "  sudo ip route add 192.168.13.0/24 via " << m_ingressAddr
            << " dev " << m_ingressDevice << "\n"
            << "Terminal A: ./ns3 run mesh-emu-m13 -- --enable-ping=0 --pcap=0\n"
            << "Terminal B: ping " << m_pingRemote << "\n"
...
MeshM13Test::InstallApplication ()
{
  if (m_rtpLocalParse)
  ...
  else
    {
      ...
      if (m_rtpIngressStats)
        {
          std::cout << "RTP ingress stats on node " << m_ingressNode
                    << ": FPS + H.264 I-frame (IDR) -> CSV " << m_rtpStatsCsv << "\n";
        }
    }
...
MeshM13Test::Run ()
{
  ...
  Simulator::Run ();
  g_ingressRtpStats.StopAndWriteCsv ();
  Simulator::Destroy ();
  return 0;
}
